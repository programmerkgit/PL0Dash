...
Token checkTokenAndGetNextToken(Token t, KeyId k);    /*　t.kind==kのチェック　*/
...
int openSourceAndTarget(char *sourceFileName, char *targetFileName);    /*　ソースファイルのopen　*/
void closeSourceAndTarget();            /*　ソースファイルのclose　*/
...
void setIdKind(KindT k);     /*　現トークン(Identifier)の種類をセット（.texファイル出力のため）*/
