...
static void incrementCIndex();                 /*　目的コードのインデックスの増加とチェック　*/
static void printCode(int i);        /*　命令語の印字　*/

/* refactor neme */
int nextCodeIndex()                    /*　次の命令語のアドレスを返す　*/
{
    return cIndex + 1;
}
...
void incrementCIndex()        /*　目的コードのインデックスの増加とチェック　*/
{
    if (++cIndex < MAXCODE)
        return;
...
void execute()            /*　目的コード（命令語）の実行　*/
{
    ...
    top = 0;
    pc = 0;            /*　top:次にスタックに入れる場所、pc:命令語のカウンタ　*/
    stack[0] = 0;
    stack[1] = 0;    /*　stack[top]はcalleeで壊すディスプレイの退避場所　*/
