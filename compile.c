...
void constDecl()            /*　定数宣言のコンパイル　*/
{
    ...
    while (1) {
        if (token.kind == Identifier) {
            ...
            temp = token;                    /*　名前を入れておく　*/
            token = checkTokenAndGetNextToken(nextToken(), Equal);        /*　名前の次は"="のはず　*/
            if (token.kind == Num)
        ...
        if (token.kind != Comma) {        /*　次がコンマなら定数宣言が続く　*/
            if (token.kind == Identifier) {        /*　次が名前ならコンマを忘れたことにする　*/
                errorInsert(Comma);
                continue;
            } else
                break;
    ...
    token = checkTokenAndGetNextToken(token, Semicolon);        /*　最後は";"のはず　*/
}
...
void funcDecl()            /*　関数宣言のコンパイル　*/
{
    ...
    if (token.kind == Identifier) {
        ...
        fIndex = enterTfunc(token.u.id, nextCodeIndex());        /*　関数名をテーブルに登録　*/
        /*　その先頭番地は、まず、次のコードの番地nextCode()とする　*/
        token = checkTokenAndGetNextToken(nextToken(), Lparen);
        blockBegin(FIRSTADDR);    /*　パラメタ名のレベルは関数のブロックと同じ　*/
        while (1) {
            if (token.kind == Identifier) {            /*　パラメタ名がある場合　*/
            ...
            if (token.kind != Comma) {        /*　次がコンマならパラメタ名が続く　*/
                if (token.kind == Identifier) {        /*　次が名前ならコンマを忘れたことに　*/
                    errorInsert(Comma);
                    continue;
                } else
                    break;
        ...
        token = checkTokenAndGetNextToken(token, Rparen);        /*　最後は")"のはず　*/
    ...
    while (1) {
        switch (token.kind) {
            ...
            case If:                    /*　if文のコンパイル　*/
                token = nextToken();
                condition();                    /*　条件式のコンパイル　*/
                token = checkTokenAndGetNextToken(token, Then);        /*　"then"のはず　*/
            ...
            case While:                /*　while文のコンパイル　*/
                token = nextToken();
                backP2 = nextCodeIndex();            /*　while文の最後のjmp命令の飛び先　*/
                condition();                /*　条件式のコンパイル　*/
                token = checkTokenAndGetNextToken(token, Do);    /*　"do"のはず　*/
                ...
                if (token.kind == Lparen) {
                    ...
                    if (token.kind != Rparen) {
                        for (;;) {
                            ...
                            token = checkTokenAndGetNextToken(token, Rparen);
                            break;
                        }
                    } else
                        token = nextToken();
